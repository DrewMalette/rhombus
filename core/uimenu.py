...
# bindings = [ ["Inventory", draw_inventory] ... etc., ]
# bindings to consolidate and replace func_dict and labels

# declare this before UI_LiveMenu
class UI_SubmenuPane:

	def __init__(self, uid, game_obj, size, func_dict): # func_dict keys coorespond to a list in UI_LiveMenu
...
class UI_LiveMenu:
	...
	def __init__(self, uid, game_obj, loc, size, child, labels):
